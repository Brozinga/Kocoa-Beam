...
实时热插拔切换、能区分多个镜头的选择器、旋转控制，以及分辨率设置（低/中/
高）。在网络较差时推流也更稳定：单个观看者网速慢不会再拖垮所有人的画面（按
观看者做背压控制），JPEG 画质现在会根据网络实际承载能力自动调整，而不是固定
...
**设置 → 远程访问 → Obico** 运行真实的 [Obico](https://www.obico.io)
（`moonraker-obico`）伴生程序，源码来自上游并经过改造，以独立进程的形式运行
在 Android 上，而不是它通常安装的 systemd 服务。它会连接到 Obico Cloud 或自
建的 Obico Server——取决于 **Obico 服务器**中设置的地址——通过与
Fluidd/Mainsail 相同的本地 Moonraker 连接，Moonraker 一侧无需额外配置。

- 打开开关会立即启动伴生程序。**关联打印机**会显示伴生程序自己生成的验证码
  （与 Obico 自身安装脚本给自行安装的 Klipper 打印机使用的流程完全一致，只是
  不需要终端）——在 Obico 应用/网站添加打印机时输入这个验证码，或者直接点击
  **打开链接**跳转到 Obico 自己的关联页面，验证码已经预先填好。对话框里也有
  一个手动验证码输入框，用于反过来的情况（Obico 给你的验证码）。**已针对真实
  的 Obico Cloud 服务器完成端到端验证**，包括通过生成验证码的方式真正完成打
  印机关联。
- 它自带的崩溃遥测（Sentry）默认已禁用，对于任何自建服务器也会自动禁用，与这
  项设置无关；与所选服务器的实际连接不受影响。
- 这里只提供周期性（约 10 秒一次）的摄像头快照，不提供 Obico 的实时 WebRTC
  预览——那需要原生的 `janus-gateway` 进程和 `ffmpeg`，两者在上游项目中都是为
  桌面 Linux/树莓派编译的，本项目未包含。详见 [`obico.md`](obico.md)。
- 即使同时运行多个打印机配置，同一时间也只有一个配置能关联到 Obico。
