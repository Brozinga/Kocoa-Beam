...
- 打开开关会启动伴生程序（在关联之前它什么都不做，只是等待——没有局域网发现
  流程，也没有交互式终端提示，这与 Obico 自身安装脚本通常的关联方式不同）。
  **关联打印机**会打开一个对话框，要求输入 6 位验证码，该验证码通过在 Obico
  应用/网站上手动添加打印机获得；输入后会用它换取与 Obico 原生关联流程相同的
  auth token。**这个 API 调用已经针对真实的 Obico Cloud 服务器验证过。**
